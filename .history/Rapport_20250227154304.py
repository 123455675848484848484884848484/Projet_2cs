...
def extract_cost_data(file_path):
    try:
        ...
        # Recherche de Daily Cost (ligne 57, colonne 77+)
        for row_idx, row in df.iterrows():
            for col_idx, cell in enumerate(row):
                if isinstance(cell, str) and "Daily Cost" in cell:
                    ...
                    raw_value = None
                    for shift in range(1, 20):  # Recherche jusqu'à 20 colonnes après
        ...
        # 🔹 Forcer l'extraction de Cumulative Cost (ligne 58, colonne 87)
        try:
            cumulative_cost_value = clean_numeric_value(df.iloc[58, 87])
            print(f"✅ Valeur Cumulative Cost détectée : {cumulative_cost_value}")
        except:
            print("⚠️ Erreur en accédant à la valeur de Cumulative Cost.")

...
def extract_blue_operations(file_path, sheet_index=1):
    ...
    :return: Liste des opérations et total des coûts.
    """
    ...
    # Supposons que la première colonne contient les noms des opérations
    operations = df.iloc[:, 0].dropna()

    # Supposons que la dernière colonne contient les coûts
    cost_column_index = -1  # Dernière colonne
    costs = df.iloc[:, cost_column_index].apply(pd.to_numeric, errors='coerce')

    # Filtrer les opérations qui ont un coût valide
    valid_operations = df.loc[costs.notna(), [df.columns[0], df.columns[cost_column_index]]]
    
    # Formater et afficher les opérations avec les coûts
    print("Opérations :")
    for index, row in valid_operations.iterrows():
        print(f"{row.iloc[0]:<30} {row.iloc[1]:,.2f}".replace(",", " "))  # Format avec espace insécable

    # Calculer le total des coûts
    total_cost = costs.sum()
    
    print(f"\nTotal : {total_cost:,.2f}".replace(",", " "))


file_path = "2.xlsx" 
#data = extract_data(file_path)
print("Données extraites :", extract_data(file_path))
extract_blue_operations(file_path)

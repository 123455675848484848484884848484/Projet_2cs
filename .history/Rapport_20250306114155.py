...
def extract_cost_data(file_path):
    try:
        ...
        # Recherche améliorée de BIT SIZE
        for row_idx, row in df.iterrows():
            for col_idx, cell in enumerate(row):
                if isinstance(cell, str) and "BIT" in cell.upper():
                    # Vérifier si "SIZE" est immédiatement après
                    if col_idx + 1 < df.shape[1] and isinstance(df.iloc[row_idx, col_idx + 1], str) and "SIZE" in df.iloc[row_idx, col_idx + 1].upper():
                        # Chercher la valeur sur plusieurs lignes en dessous

...
def clean_numeric_value(value):
    ...
    if isinstance(value, str):
        value = value.replace("\xa0", "").replace("\u202f", "").replace(" ", "").replace("US$", "").strip()  # Supprime les espaces insécables et la devise
...
def extract_cost_data(file_path):
    try:
        ...
        df = pd.read_excel(file_path, sheet_name=0, header=None, dtype=str, na_filter=False)
                ...
                if isinstance(cell, str):
                    ...
                    if "Daily Cost" in cell_clean and col_idx + 1 < df.shape[1]:
                        ...
                        if pd.isna(raw_value) or raw_value is None:
                            print(f"⚠️ Valeur vide ou NaN pour Daily Cost à la ligne {row_idx}, colonne {col_idx+1}")
                        else:
                    ...
                    if "Cumulative Cost" in cell_clean and col_idx + 1 < df.shape[1]:
                        ...
                        if pd.isna(raw_value) or raw_value is None:
                            print(f"⚠️ Valeur vide ou NaN pour Cumulative Cost à la ligne {row_idx}, colonne {col_idx+1}")
                        else:

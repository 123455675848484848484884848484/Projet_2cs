...
const FichierJournalier = () => {
  ...
  const [openRows, setOpenRows] = useState([true]);
  ...
  const handleSubmit = () => {
    ...
    console.log("Fichiers:", selectedFiles);
  };
  ...
  return (
    <div className="min-h-screen px-24 py-12 bg-[#f4f4f4]">
      ...
      <div className="flex flex-col gap-2 mb-10">
        <label className="bg-[#2f5744] cursor-pointer text-white text-[20px] font-semibold py-5 rounded-md flex justify-center items-center gap-4">
          <img src="/file.png" alt="icon" className="w-6 h-6" />
          Ajouter un fichier journalier
          <input type="file" multiple className="hidden" onChange={handleFileChange} />
        </label>
        {selectedFiles.length > 0 && (
          <ul className="text-sm text-gray-700 pl-2 list-disc">
                ...
                <td className="px-4 py-2 align-top">
                  ...
                  >
                    <span className="text-gray-800">
                      {row.operation ? row.operation : "Sélectionner une opération"}
                    </span>
      ...
      <textarea
        className="w-full h-24 border border-gray-300 rounded-md p-3 mb-8 focus:outline-none focus:ring-0 focus:border-orange-600"
        placeholder=""

...
const FichierJournalier = () => {
  ...
  return (
    <div className="min-h-screen px-24 py-12 bg-[#f4f4f4]">
              ...
              <tr key={index} className="bg-white rounded-md">
                <td className="px-4 py-2 align-top">
                  <div className="flex items-center justify-between cursor-pointer" onClick={() => {
                    ...
                    <span className="text-gray-800">
                      {row.operation.length > 0 ? row.operation.join(", ") : "Sélectionner une opération"}
                    </span>
                          ...
                          {group.items.map((item) => (
                            <label key={item} className="block text-sm text-gray-800">
                              <input
                                ...
                                onChange={() => handleChange(index, "operation", [item])}
                                className="mr-2"
                              />
                              {item}
                            </label>
                ...
                <td className="px-4 py-2 align-top">
                  <input
                    ...
                    className="w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-0 focus:border-orange-500"
                  />
                </td>
                <td className="px-4 py-2 align-top">
                  <input
                ...
                <td className="px-4 py-2 align-top">
                  <label className="bg-green-700 text-white rounded px-4 py-2 cursor-pointer inline-block">
                    + Ajouter
                    <input
                      type="file"
                  ...
                  {row.file && (
                    <p className="mt-2 text-sm text-green-700">{row.file.name}</p>
                  )}
                </td>
              </tr>
            ))}
      ...
      <textarea
        className="w-full h-24 border border-gray-300 rounded-md p-3 mb-8 focus:outline-none focus:ring-0 focus:border-orange-500"
        placeholder=""

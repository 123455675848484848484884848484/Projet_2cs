...
const FichierJournalier = () => {
  ...
  const [openRows, setOpenRows] = useState([false]); // initialise avec une ligne fermée
  ...
  const handleAddRow = () => {
    ...
    setOpenRows([...openRows, false]); // ajouter un état fermé
  };
    ...
    <React.Fragment key={index}>
      ...
      <tr className="bg-white rounded-md">
        <td className="px-4 py-2">
          <button
            onClick={() => {
              const updatedOpen = [...openRows];
              updatedOpen[index] = !updatedOpen[index];
              setOpenRows(updatedOpen);
            }}
      ...
      {/* Contenu déroulant conditionnel */}
      {openRows[index] && (
        <tr>
          <td colSpan="3" className="px-4 py-4 bg-gray-100 rounded-b">
                    ...
                    <label key={item} className="block text-sm text-gray-800">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const updatedOps = row.operation.includes(value)
                            ? row.operation.filter((op) => op !== value)
                            : [...row.operation, value];
                          handleChange(index, "operation", updatedOps);
                        }}
                        className="mr-2"
                      />
                      {item}

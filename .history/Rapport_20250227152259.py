            ...
            for col_idx, cell in enumerate(row):
                if isinstance(cell, str):
                    ...
                    if "Daily Cost" in cell_clean and col_idx + 1 < df.shape[1]:
                        ...
                        raw_value = None
                        # Essayer 3 colonnes à droite, puis chercher sur les lignes en dessous
                        ...
                        # Si toujours vide, vérifier sur les 3 lignes suivantes
                        if not raw_value:
                            print(f"🔎 Vérification des lignes suivantes (lignes {row_idx+1} à {row_idx+3}):")
                            for next_row in range(1, 4):
                                if row_idx + next_row < df.shape[0]:
                                    temp_value = df.iloc[row_idx + next_row, col_idx + 1]  # Vérifie dans la même colonne
                                    print(f"📌 Ligne {row_idx + next_row}, Colonne {col_idx + 1} : {repr(temp_value)}")
                                    if temp_value.strip():
                                        raw_value = temp_value
                                        break
